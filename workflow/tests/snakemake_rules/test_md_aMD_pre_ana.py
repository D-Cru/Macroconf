...
def test_md_aMD_pre_ana():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath("tests/snakemake_rules/md_aMD_pre_ana/data")
        expected_path = PurePosixPath(
            "tests/snakemake_rules/md_aMD_pre_ana/expected"
        )
        ...
        # dbg
        print(
            "data/interim/tests/24/H2O/10_aMD_prod/2/0/1b5ac9a7d3a7b8e5_weights.dat",
            file=sys.stderr,
        )
        ...
        to_delete = [
            "samples_old.tsv",
            "samples_tests.tsv",
            "data/interim/tests/24/H2O/10_aMD_prod/2/0/1b5ac9a7d3a7b8e5_pre_ana.log",
        ]
        for d in to_delete:

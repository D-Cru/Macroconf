...
def test_omega_first_flip():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath(
            "tests/snakemake_rules/omega_first_flip/data"
        )
        expected_path = PurePosixPath(
            "tests/snakemake_rules/omega_first_flip/expected"
        )
        ...
        # dbg
        print("data/interim/tests/24/omega.smi", file=sys.stderr)
        ...
        sp.check_output(
            [
                ...
                "-m",
                "snakemake",
                "data/interim/tests/24/omega.smi",
                "-f",
                "-j1",
                "--keep-target-files",
                "--configfile",
        ...
        to_delete = [
            "samples_old.tsv",
            "samples_tests.tsv",
            "data/interim/tests/24/first_flip.log",
        ]
        for d in to_delete:

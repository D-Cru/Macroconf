...
def test_omega_extract_mol_pdb():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath(
            "tests/snakemake_rules/omega_extract_mol_pdb/data"
        )
        expected_path = PurePosixPath(
            "tests/snakemake_rules/omega_extract_mol_pdb/expected"
        )
        ...
        # dbg
        print(
            "data/interim/tests/24/omega/basic/mcs.pdb data/interim/tests/24/omega/basic/conf_energies.txt",
            file=sys.stderr,
        )
        ...
        sp.check_output(
            [
                ...
                "-m",
                "snakemake",
                "data/interim/tests/24/omega/basic/mcs.pdb",
                "data/interim/tests/24/omega/basic/conf_energies.txt",
                "-f",
                "-j1",
                "--keep-target-files",
                "--configfile",
        ...
        to_delete = [
            "samples_old.tsv",
            "samples_tests.tsv",
            "data/interim/tests/24/omega/basic/extract_mol_pdb.log",
        ]
        for d in to_delete:

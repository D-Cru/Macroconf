...
def test_md_aMD_make_param():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath(
            "tests/snakemake_rules/md_aMD_make_param/data"
        )
        expected_path = PurePosixPath(
            "tests/snakemake_rules/md_aMD_make_param/expected"
        )
        ...
        # dbg
        print(
            "data/interim/tests/24/H2O/10_aMD_prod/2/0/1b5ac9a7d3a7b8e5_aMD.in",
            file=sys.stderr,
        )

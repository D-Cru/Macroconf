...
def test_md_comp_stats():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath("tests/snakemake_rules/md_comp_stats/data")
        expected_path = PurePosixPath(
            "tests/snakemake_rules/md_comp_stats/expected"
        )
        ...
        # dbg
        print(
            "data/processed/tests/results/24/comparison/heatmap.png",
            file=sys.stderr,
        )
        ...
        to_delete = [
            "samples_old.tsv",
            "samples_tests.tsv",
            "data/processed/tests/notebooks/24/comp_stats_log.ipynb",
        ]
        for d in to_delete:

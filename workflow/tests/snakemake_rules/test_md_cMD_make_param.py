...
def test_md_cMD_make_param():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath(
            "tests/snakemake_rules/md_cMD_make_param/data"
        )
        expected_path = PurePosixPath(
            "tests/snakemake_rules/md_cMD_make_param/expected"
        )
        ...
        # dbg
        print(
            "data/interim/tests/24/H2O/8_cMD/2/0/214b8ec88026de74_cMD.in",
            file=sys.stderr,
        )
        ...
        to_delete = [
            "samples_old.tsv",
            "samples_tests.tsv",
            "data/interim/tests/24/H2O/8_cMD/2/0/214b8ec88026de74_cMD_makeparam.log",
        ]
        for d in to_delete:

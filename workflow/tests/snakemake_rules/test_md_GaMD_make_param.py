...
def test_md_GaMD_make_param():

    with TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        data_path = PurePosixPath(
            "tests/snakemake_rules/md_GaMD_make_param/data"
        )
        expected_path = PurePosixPath(
            "tests/snakemake_rules/md_GaMD_make_param/expected"
        )
        ...
        # dbg
        print(
            "data/interim/tests/24/H2O/11_GaMD_full/2/0/7f5696daa1cbe87d_GaMD.in",
            file=sys.stderr,
        )
        ...
        to_delete = [
            "samples_old.tsv",
            "samples_tests.tsv",
            "data/interim/tests/24/H2O/11_GaMD_full/2/0/7f5696daa1cbe87d_GaMD_makeparam.log",
        ]
        for d in to_delete:

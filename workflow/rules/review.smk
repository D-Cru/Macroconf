...
rule md_GaMD_sol_prop_mc_rew:
    ...
    output:
        ...
        noe_stats=f"data/processed/review/results/{{compound_dir}}/{{solvent}}/GaMD/{{time}}/{{repeat}}/{{index}}_noe_stats_mc.json",
        noe_result=f"data/processed/review/results/{{compound_dir}}/{{solvent}}/GaMD/{{time}}/{{repeat}}/{{index}}_noe_result_mc.json",
        sasa=f"data/processed/review/results/{{compound_dir}}/{{solvent}}/GaMD/{{time}}/{{repeat}}/{{index}}_sasa_mc.pkl",
        psa=f"data/processed/review/results/{{compound_dir}}/{{solvent}}/GaMD/{{time}}/{{repeat}}/{{index}}_psa_mc.pkl",
        solvation_properties=f"data/processed/review/results/{{compound_dir}}/{{solvent}}/GaMD/{{time}}/{{repeat}}/{{index}}_solvation_properties.json",
    params:
        cluster_dir=f"data/processed/{config['exp_name']}/results/{{compound_dir}}/{{solvent}}/GaMD/{{time}}/{{repeat}}/{{index}}_clusters/",
        rst_dir=f"data/processed/{config['exp_name']}/results/{{compound_dir}}/{{solvent}}/GaMD/{{time}}/{{repeat}}/{{index}}_clusters/rst/",
        method="GaMD",
...
rule md_cMD_sol_prop_mc_rew:
    ...
    output:
        ...
        noe_stats=f"data/processed/review/results/{{compound_dir}}/{{solvent}}/cMD/{{time}}/{{repeat}}/{{index}}_noe_stats_mc.json",
        noe_result=f"data/processed/review/results/{{compound_dir}}/{{solvent}}/cMD/{{time}}/{{repeat}}/{{index}}_noe_result_mc.json",
...
def solv_accumulate(wildcards):
    ...
    for sim_hash in simulation_ids:
        ...
        output[
            f"run_{sim_hash}"
        ] = f"data/processed/review/results/{w.compound}/{w.solvent}/{w.method}/{w.simtime}/{w.repeats}/{sim_hash}_solvation_properties.json"
...
def comp_methods_input_mc(wildcards):
    output = {}
    if wildcards.igamd == "nan" or wildcards.igamd == "3":
        igamd_parameters = ["nan", "3"]
    else:
    ...
    for sim_hash in simulation_ids:
        ...
        output[
            f"run_{sim_hash}"
        ] = f"data/processed/review/results/{w.compound}/{w.solvent}/{w.method}/{w.simtime}/{w.repeats}/{sim_hash}_noe_stats_mc.json"

    return output


# Aggregate 1 method type (e.g. all cMD,2k ns with exactly the same parameters)
rule md_comp_methods_review_mc:
    input:
        unpack(comp_methods_input_mc),
    output:
        plot=report(
            "data/processed/review/results/methods/{method}-{solvent}-{simtime}-{igamd}-NOE_mc.png"
        ),
        data="data/processed/review/results/methods/{method}-{solvent}-{simtime}-{igamd}-NOE_fulfilled_mc.json",
    log:
        notebook="data/processed/review/notebooks/methods/{method}-{solvent}-{simtime}-{igamd}-NOE_method_comp_mc.ipynb",
    conda:
        "../envs/stats.yaml"
    threads: 2
    notebook:
...
rule md_comp_reweighting_mc:
    input:
        unpack(comp_all_methods_input_review),
        mc="data/processed/review/results/methods/GaMDmc-native-2000-3-NOE_fulfilled_mc.json",
    output:
        plot1=report(
        ...
        plot2=report(
            "data/processed/review/results/methods/{methods}_{conf_gens}-NOE-all_2_rw.svg"
        ),
        plot2_sig=report(
            "data/processed/review/results/methods/{methods}_{conf_gens}-NOE-all_sig_2_rw.svg"
        ),
    log:

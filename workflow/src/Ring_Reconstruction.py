...
def GetCoordinate(mol, ringpath):
    ...
    """
    coordinate = [
        list(mol.GetConformer().GetAtomPosition(x)) for x in ringpath
    ]
    return coordinate
...
def SetInitialRingBondLength(mol, ringpath):
    ...
    ele_bond = [
        (sorted_ele[i][0], sorted_ele[i][1], bond[i])
        for i, j in enumerate(bond)
    ]
    bondlength = [
        bondtable.BONDLENGTH_REF.get(
            eb, "Unknown bond length, please update BOND LENGTH table"
        )
        for eb in ele_bond
    ]
    return bondlength
...
def GetRingBondAng(mol, ringpath):
    ...
    N = len(ringpath)
    atoms = [
        [ringpath[i], ringpath[(i + 1) % N], ringpath[(i + 2) % N]]
        for i in range(N)
    ]
    molconf = mol.GetConformer()
    bondang = [
...
def SetInitialRingBondAng(mol, ringpath):
    ...
    for a in atoms:
        bond.append(
            [
                int(mol.GetBondBetweenAtoms(a[i], a[i + 1]).GetBondType())
                for i in range(2)
            ]
        )
    elements_and_bond_order = [
        sorted_ele[i] + tuple(bond[i]) for i, j in enumerate(bond)
    ]
    bondang = [
        bondtable.BONDANGLE_REF.get(
            x, "Unknown bond length, please update BOND ANGLE table"
        )
    ...
    if ringsize % 2 == 1:  # odd ring size
        for j in range(ringsize):
            tmp = np.sum(
                [
                    q[m - 2]
                    * np.cos(ang[m - 2] + 2 * np.pi * m * j / ringsize)
                    for m in range(2, K)
                ]
            )
...
def UpdateBondLength(ringsize, Z, init_bondl):
    ...
    """
    assert (
        len(Z) == ringsize and len(init_bondl) == ringsize
    ), "Inappropriate Input"
    N = ringsize
    nr = [
        np.sqrt(np.square(init_bondl[i]) - np.square(Z[(i + 1) % N] - Z[i]))
        for i in range(N)
    ]
    return nr
...
def UpdateBeta(ringsize, Z, r, beta):
    ...
    """
    assert (
        len(Z) == ringsize and len(r) == ringsize and len(beta) == ringsize
    ), "Inappropriate Input"
    N = ringsize
    ...
    for i in idxlist:
        tmp_a = (
            np.square(Z[i[2]] - Z[i[0]])
            - np.square(Z[i[1]] - Z[i[0]])
            - np.square(Z[i[2]] - Z[i[1]])
        )
...
def SegCoord(segment, r, beta):
    ...
    if segsize >= 3:
        for index in range(segsize - 2):
            ...
            alpha.append(
                segment_beta[index + 1]
                - np.arcsin(
                    np.sin(segment_beta[index])
                    * slength[index]
                    / slength[index + 1]
                )
            )
...
def RingPartition(ringsize, z, r, beta):
    ...
    assert (
        ringsize <= 16 and ringsize >= 5
    ), "Ring size greater than 16 or smaller than 5 is not supported"
    ...
    phi1, phi2, phi3 = (
        np.arctan2(segcoord_1[-1][1], segcoord_1[-1][0]),
        np.arctan2(segcoord_2[-1][1], segcoord_2[-1][0] - np.sqrt(OQsq)),
        np.arctan2(segcoord_3[-1][1], segcoord_3[-1][0]),
    )
    ...
    Rsigma1, Rsigma2, Rsigma3 = (
        RotationMatrix(-sigma1),
        RotationMatrix(sigma2),
        RotationMatrix(-sigma3),
    )
    ...
    newcoord = [
        np.matmul(Rphig, coordinate[i] - Rg).tolist() + [z[i]]
...
####################################################
def SetRingPuckerCoords(
    mol, ringpath, amplitude, angle, init_bondl, init_bondang
):
    """
